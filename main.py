...
from datetime import (
    timedelta,
)  # Explicitly imported as a dependency for other modules like scraper.py if date parsing is involved
...
async def main():
    ...
    all_found_jobs = []
    driver = (
        None  # Ensure driver is initialized to None for proper cleanup in finally block
    )
    ...
    try:
        ...
        if TELEGRAM_BOT_TOKEN is None:
            logger.critical(
                "TELEGRAM_BOT_TOKEN is not configured. Aborting job search."
            )
            return
        ...
        for website_config in JOB_WEBSITES:
            logger.debug(
                f"Attempting to scrape from: {website_config['name']} ({website_config['url']})"
            )
            jobs_from_site = scrape_jobs_from_website(driver, website_config)
            all_found_jobs.extend(jobs_from_site)
            logger.debug(
                f"Found {len(jobs_from_site)} jobs from {website_config['name']}."
            )
        ...
        for job in all_found_jobs:
            if is_job_posting(
                job["title"], job["description"], JOB_TITLE_KEYWORDS, JOB_KEYWORDS
            ):
                logger.debug(
                    f"Job identified as relevant: {job['title']} (Link: {job['link']})"
                )
                if job["link"] not in posted_job_links:
                    new_relevant_job_postings.append(job)
                else:
                    logger.info(
                        f"Skipping duplicate job: '{job['title']}' (Link: {job['link']}) - already notified."
                    )
            else:
                logger.debug(
                    f"Job NOT relevant based on criteria: '{job['title']}' (Source: {job['source']})"
                )

        # Handle notification logic based on findings.
        if not new_relevant_job_postings:
            logger.info(
                "No NEW relevant job postings found in this cycle. Sending an informational message."
            )
            ...
            logger.info(
                f"Identified {len(new_relevant_job_postings)} NEW relevant job postings. Initiating Telegram notifications..."
            )
            # Send each new relevant job and record its link to prevent future duplicates.
            for job_post in new_relevant_job_postings:
                logger.debug(
                    f"Attempting to send Telegram message for: '{job_post['title']}'"
                )
                ...
                if message_sent:
                    ...
                    posted_job_links.add(
                        job_post["link"]
                    )  # Update in-memory set to prevent duplicates within the same run
                    logger.debug(
                        f"Successfully sent and recorded job link: {job_post['link']}"
                    )
                else:
                    logger.warning(
                        f"Failed to send Telegram message for '{job_post['title']}'. This job will NOT be marked as posted and may be resent."
                    )

    except Exception as e:
        # Catch any unexpected exceptions during the main execution flow.
        logger.critical(
            f"An unhandled exception occurred during main execution: {e}", exc_info=True
        )
    finally:
        ...
        if driver:
            try:
                ...
                logger.warning(
                    f"WebDriverException during driver.quit(): {e}. This might indicate the browser process was already closed or an invalid session."
                )
            except OSError as e:
                logger.warning(
                    f"OSError during driver.quit() (e.g., 'Handle invalid' on Windows): {e}. This suggests a system-level issue with process management."
                )
            except Exception as e:
                logger.warning(
                    f"An unforeseen error occurred while attempting to quit the driver: {e}"
                )
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.info(
            "Script execution interrupted by user (KeyboardInterrupt). Shutting down gracefully."
        )
    except Exception as e:
        # Catch any exceptions occurring outside the main async function call.
        logger.critical(
            f"An unexpected error occurred during script initialization or shutdown: {e}",
            exc_info=True,
        )
